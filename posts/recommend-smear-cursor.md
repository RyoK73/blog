...
[smear-cursor](https://github.com/sphamba/smear-cursor.nvim)のこと。
NeoVimのプラグインの一つで、執筆時点で1800スターを獲得している。
[NeoVide](https://neovide.dev)のようなカーソルアニメーションを追加することができる。
かなり細かくカスタマイズできるので、通常のNeoVimの見た目に飽きた人にもおすすめ！

### 導入時の注意点

- [mini-animate](https://github.com/nvim-mini/mini.animate)とは動作が似てて競合するのでどっちかにする必要がある
  (mini-animateはスクロールの際にページの下側でスタックすることがあったので、smear-cursorのほうが個人的にはおすすめ)
- アニメーションなので、グラフィック性能に余裕がない環境だと重くなるかもしれない汗

## 設定ファイルの書き方

`nvim/lua/plugins/smear-cursor.lua`に以下のように記述する。
※ ファイル名はなんでもOK
